...
namespace flopse
{
	...
	{
		boundingBox = new BoundingBox(glm::vec3(), 0.f, 0.f, 0.f);
	}

	ICollidable::ICollidable(const glm::vec3 &pos, float width, float height, float depth)
	{
		boundingBox = new BoundingBox(pos, width, height, depth);
	}
